...
6. **Phase 6: contract registry.** Reconcile the full live repository estate and current services. Gate: manifest, source, metadata, and live observations report explicit drift; no public route regression.
...
9. **Manifest coverage for support/content repos?** Recommended: register the full live estate in a repository inventory, but only runtime services receive public route contracts; do not invent endpoints for libraries or content tools.
