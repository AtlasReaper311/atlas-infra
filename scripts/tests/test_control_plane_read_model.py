...
class ControlPlaneReadModelTests(unittest.TestCase):
    ...
    def build(self, **overrides):
        ...
        return read_model.build_read_model(**arguments)

    def test_build_is_deterministic_schema_valid_and_matches_fixture(self):
        first, first_receipt = self.build()
        second, second_receipt = self.build()
        expected = ROOT / "tests/fixtures/control-plane-read-model/expected"
        self.assertEqual(first, second)
        self.assertEqual(first_receipt, second_receipt)
        self.assertEqual([], validate_instance(first, load_json(self.schema)))
        self.assertEqual(load_json(expected / "read-model.json"), first)
        self.assertEqual(load_json(expected / "dry-run-receipt.json"), first_receipt)
        self.assertEqual(21, len(first["sources"]))
        self.assertEqual("control-plane:read-model:v1", first_receipt["bounded_kv_key"])
        self.assertFalse(first_receipt["provider_write_performed"])

    def test_fingerprints_cover_sources_and_complete_model(self):
        model, _ = self.build()
        source_digest = hashlib.sha256(canonical_json(model["sources"]).encode()).hexdigest()
        self.assertEqual(f"sha256:{source_digest}", model["source_fingerprint"])
        payload = copy.deepcopy(model)
        actual = payload.pop("read_model_fingerprint")
        model_digest = hashlib.sha256(canonical_json(payload).encode()).hexdigest()
        self.assertEqual(f"sha256:{model_digest}", actual)
    ...
    def test_secret_bearing_and_machine_local_values_are_rejected(self):
        for field, value, error in (
            ("token", "fixture", "forbidden or secret-bearing key"),
            ("display_name", "http://127.0.0.1:8123", "machine-local"),
        ):
            with tempfile.TemporaryDirectory() as directory:
                target = Path(directory) / "collections"
                shutil.copytree(self.collection_sources, target)
                ...
                with self.assertRaisesRegex(read_model.ProducerError, error):
                    self.build(collection_sources=target)
    ...
    def test_expired_collection_becomes_stale(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            receipt = next(item for item in model["sources"] if item["source_id"] == "services")
            self.assertEqual("stale", model["services"][0]["state"])
            self.assertEqual("stale", receipt["state"])
    ...
    def test_collection_producer_and_dependencies_must_be_public(self):
        for mutate, error in (
            (lambda document: document.update(producer="AtlasReaper311/private-repository"), "producer is not public"),
            (lambda document: document["items"][0]["dependencies"].append("private-service"), "non-public dependency identity"),
        ):
            with tempfile.TemporaryDirectory() as directory:
                target = Path(directory) / "collections"
                shutil.copytree(self.collection_sources, target)
